...
type ColumnType = ColumnDef<ArticleInterface, unknown>;
...
export function ArticleListPage(props: ArticleListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ArticleInterface>>(
    () => `/articles?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/articles/view/${record.id}`} passHref legacyBehavior>
            <Button
